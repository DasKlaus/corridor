...
function drawCell(data, svg) {
	...
	var cell = d3.select("svg[index='"+index+"'] .chart").append("circle")
		...
		.attr("limit", function(){
			// copy limits from other cells of same row
			var limit = d3.select(".cell[cellid='"+row+"']").attr("limit");
			limit = (limit==null || limit=="")?new Array():limit.split(" ");
			// TODO: why is this returning an array with an empty string? Still buggy
			if (limit.length>0) {
			...
			if (y<top || y>bottom) {
				...
				d3.selectAll(".cell[cellid='"+row+"']").attr("limit",limit.join(" ")).style("fill",inactiveCell);
			}
			return limit.join(" ");
		})
...
function drawColumn(select) {
	...
	slide = d3.behavior.drag()
		...
		d3.select(this.parentNode).selectAll(".cell").each(function() {
			...
			var limits = d3.select(this).attr("limit").split(" "); // existing limits for this cell
			var cells = d3.selectAll(".cell[cellid='"+cellid+"']"); // get all cells belonging to same row			
			if (limited && limits.indexOf(index)<0) {
				limits.push(index); // add limit if not set
				cells.attr("limit", limits.join(" ")).style("fill", inactiveCell); // set limit and color
			}
			if (!limited && limits.indexOf(index)>=0) {
				limits = limits.splice(limits.indexOf(index),1); // remove limit if set
				cells.attr("limit", limits.join(" ")).style("fill", activeCell); // set limit and color
			}
		});
